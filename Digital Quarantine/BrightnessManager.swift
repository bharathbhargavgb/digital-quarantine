...
internal class BrightnessManager: NSObject {
    ...
    internal class ExternalDisplay: Display {
        ...
        override init(fromDisplay displayID: CGDirectDisplayID) {
            ...
            if BrightnessManager.shared.readBrightnessBeforeChanging.boolValue {
                ...
                // TODO: read from config file for default initial value
                // let location = NSString(string: "~/.config/DQ/settings").expandingTildeInPath
    ...
    func getBrightness() -> [CGDirectDisplayID: Float] {
        var displayDict = [CGDirectDisplayID: Float]()
        for displayID in type(of: self).getAllDisplayIDs() {
            displayDict[displayID] = self.getDisplay(withID: displayID).getBrightness()
        }
        return displayDict
    }
    
    func setBrightnessLevels(displayDict: [CGDirectDisplayID: Float]) {
